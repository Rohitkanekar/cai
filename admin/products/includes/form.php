            ...
            <div class="col-md-6 mb-3">
                ...
                <select name="category_id" id="category_id" class="form-select" required>
                    ...
                    <?php foreach ($categories as $cat): ?>
                        <option value="<?= $cat['id']; ?>" data-name="<?= htmlspecialchars($cat['name']); ?>" <?= (($product['category_id'] ?? 0) == $cat['id']) ? 'selected' : ''; ?>>
...
$singleSizeVal = $product['single_size'] ?? ($productSizes[0]['size'] ?? '');
$singlePriceVal = $product['single_price'] ?? ($productSizes[0]['price'] ?? '');
?>

    <!-- FOR PLANTERS ONLY -->

    <div class="card shadow-sm border-0 mb-4" id="planterSizesWrapper" style="display: <?= $isPlanter ? 'block' : 'none' ?>;">
        ...
        <div class="card-body">
            <?php 
            $planterSizesData = $isPlanter ? $productSizes : [[]];
            foreach ($planterSizesData as $index => $size): 
            ?>
                <div class="sizeRow card border p-3 mb-3">
                    <div class="row">
                        ...
                        <div class="col-md-1">
                            <label class="form-label">Breadth mm</label>
                            <input type="number" name="breadth_mm[]" class="form-control" value="<?= $size["breadth_mm"] ?? '' ?>">
                        </div>
                        <div class="col-md-1">
                            <label class="form-label">Breadth inch</label>
    ...
    <div class="card shadow-sm border-0 mb-4" id="normalProductWrapper" style="display: <?= !$isPlanter ? 'block' : 'none' ?>;">
        <div class="card-header bg-dark text-white d-flex justify-content-between align-items-center">
            <h5 class="mb-0">
                Product Size & Price
            </h5>            
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="single_size" class="form-control"
                        value="<?= htmlspecialchars($singleSizeVal) ?>">
                </div>
                <div class="col-md-6 mb-3">
                    <label class="form-label">Price</label>
                    <input type="number" name="single_price" class="form-control"
                        value="<?= htmlspecialchars($singlePriceVal) ?>">
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const planterWrapper = document.getElementById("planterSizesWrapper");
        const normalWrapper = document.getElementById("normalProductWrapper");
        const wrapper = document.getElementById("planterSizesWrapper");
        const addBtn = document.getElementById("addSize");

        function checkCategoryType() {
            if (!categorySelect) return false;
            const selectedOption = categorySelect.options[categorySelect.selectedIndex];
            const catName = (selectedOption.getAttribute("data-name") || selectedOption.text || "").toLowerCase();
            return catName.includes("planter");
        }

        function toggleSections() {
            const isPlanterDynamic = checkCategoryType();
            if (isPlanterDynamic) {
                if (planterWrapper) planterWrapper.style.display = "block";
                if (normalWrapper) normalWrapper.style.display = "none";
            } else {
                if (planterWrapper) planterWrapper.style.display = "none";
                if (normalWrapper) normalWrapper.style.display = "block";
            }
        }
